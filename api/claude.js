export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: { message: 'GROQ_API_KEY not set' } });
...
8. For outreach emails: unless a word limit is explicitly given, write a FULL email of 150-200 words minimum. Never write a stub or summary — write the complete email.
...
${system || ''}`;
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: { message: data.error?.message || 'Groq API error' } });
